...
with an inductive predicate (\texttt{Rpath} in \texttt{seq1.v})
...
as we have
{\small \inputsnippets{seq1/RPath_equiv.tex}}\coqend

As a first example, using the
relation $C_{\textsc{H}}$ -- denote \texttt{Chrel} in Coq -- on the product set
$T{\times}T$ defined by
$((v_1, v_2)\, C_{\textsc{H}}\, (v_3, v_4) \iff v_2 = v_3)$, that is
{\small \inputsnippetsE{erel3/Chrel}{,}} 
the fact that lifted sequences are well chained sequences is proved as 
{\small \inputsnippets{erel3/Lift_Suc.tex}}

As a second example, if the elements of a sequence belong to a set $X$, then the 
elements of the lifted sequence belong to the product relation $X{\times}X$
{\small \inputsnippets{seq1/RPath_L1.tex}}

four equivalent ways -- two are
