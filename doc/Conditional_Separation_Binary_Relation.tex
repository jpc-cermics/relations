...
\paragraph{(Coq)} We describe the coq formalization of (edge) paths.  A graph
$(\VERTEX,\EDGE)$ is given by a type \texttt{(T: Type)} and a relation on
\texttt{T} is a set in the product type \texttt{set T*T}, that is
$(\EDGE:\texttt{set T*T})$. Thus, the (classical) set definition of mathcomp
analysis \texttt{classical\_sets.v} is used to formalize a graph. Now, the lift
operator $\Lift$ is implemented in Coq as a recursive mapping denoted
\texttt{Lift}: {\small \inputsnippets{erel3/Lift.tex} } Finally, the set of
finite (edge) paths of length greater of equal to $1$,
$\PATH^{\ge 1}\np{\OrientedGraph}$ defined in Equation~\eqref{def:epaths_gt1} is
formalized in Coq using the function \texttt{all} --- used to checks that the elements of a sequence belong to a given set 
-- combined with the \texttt{Lift} operator:
{\small
  \inputsnippets{erel3/all_notation.tex,erel3/EPath1.tex}
}
To conclude, at Coq level, an (edge) path is represented by its preimage by the lift operation, that is a sequence of 
vertices which gives an edge path. 
