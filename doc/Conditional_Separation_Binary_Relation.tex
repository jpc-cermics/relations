...
ZZZZZ \cite{Yamaguchi-et-al:2016} \cite{Affeldt-et-al:2020}
...
% {\small \inputsnippets{seq1/all_notation-n.tex,seq1/Sn-n.tex}}
{\small \Snippetallnotation\SnippetSn}
\end{leftbar}
...
\begin{leftbar} The lift operator $\Lift$ is implemented in Coq as a recursive mapping
  denoted \texttt{Lift}:
  % {\small \inputsnippetsE{seq1/Lift.tex}{.} }
  {\small \SnippetLift}
  We note that,
...
% {\small \inputsnippets{seq1/RPath_equiv-n.tex}}
...
%
  ...
  \begin{leftbar}
    ...
    {\small \SnippetDI\SnippetLiftinj\SnippetLiftsurj}
  \end{leftbar}
...
{\small \SnippetEpeLift\SnippetPeUnLift}
\end{leftbar}

\myparagraph{$\bullet$ Transitive closure and edge paths.} As a first result linking
(edge) paths and relations we prove that 
  % \inputsnippetsE{seq1/TCP-n.tex}{,}
  {\small \SnippetTCP}
  which asserts that two nodes
...
\begin{leftbar}
  % {\small \inputsnippets{seq1/D_P-n.tex}}
  {\small \SnippetDP}
...
\footnote{%
  ...
  $\UPATH_{> n}$
  % {\small \inputsnippetsE{seq1/U_gt-n.tex,seq1/Oedge-n.tex,seq1/ChrelO-n.tex}{,}}
  {\small \SnippetUgt\SnippetOedge\SnippetChrelO}
...
\begin{subequations}
  ...
  \end{equation}
\end{subequations}
\begin{leftbar} The Coq definition follows
  % {\small \inputsnippetsE{seq1/Eope.tex}{,}}
  {\small \SnippetEope}
...
{\small \Snippetpair\SnippetLiftO}
\end{leftbar}
...
{\small \SnippetEopeLiftO\SnippetPeUnLiftO}
\end{leftbar}
...
{\small \Snippetdsepnota\SnippetDseparated}
%{\small \inputsnippets{seq1/Aeop-n.tex,seq1/ActiveOe-n.tex,seq1/allL-n.tex}}
{\small \SnippetAeop\SnippetActiveOe\SnippetallL}
\end{leftbar}
\begin{leftbar}
  The retained formulation is indeed equivalent to Definition~\ref{de:vertices-d-separated},
...
\begin{lemma} (\PY{k+kn}{Lemma}~\PY{n+nf}{L5})
  ...
  \begin{equation}
    \UndirectedPath = \bp{ \np{\bgent,\dgent,\orient_1},
      \np{\dgent,\cgent,\orient_2} }
    \text{ with }
...
% {\small \inputsnippets{paper_csbr/L7_csbr-n.tex}}
...
%
Given and \EdgePath\ $\path \in \PATH\np{\OrientedGraph}$ and $o\in \Orientation^{\cardinal{\path}}$ we denote by
$\UndirectedPath=\Pair{\path,o}$ the \undirectedEdgePath\,
$\UndirectedPath \in \UPATH\np{\OrientedGraph}$, defined by
$\UndirectedPath_i = (\path_i, o_i)$ for all $i\in \ic{1, \cardinal{\path}}$.
...
\begin{lemma}
  ...
  \begin{subequations}
    \begin{align}
      ...
      & 
        \mtext{there exists } \UndirectedPath \in \UPATH\np{\OrientedGraph}
        \mtext{ such that } \UndirectedPath = \Pair{\path, \1_{\cardinal{\path}} } \mtext{ and }
        \nonumber \\
      & 
        \UndirectedPath    \in 
        \DeploymentInUPaths{\np{\bgent,\cgent}}{\OrientedGraph} 
      ...
      \implies
      & 
        \mtext{there exists }\UndirectedPath=\Pair{\path,\orient} \in
        \UPATH\np{\OrientedGraph},
        \mtext{with } \cardinal{\UndirectedPath} \geq 2
        \nonumber \\
      & \mtext{and } \orient=\np{-1,\ldots,+1} 
        ...
        \nonumber \\
      & 
        \UndirectedPath=\Pair{\path,\orient}=\Pair{\path, \np{-1,\ldots,+1} } 
        \in 
        \DeploymentInUPaths{\np{\bgent,\cgent}}{\OrientedGraph}
        \cap \ActiveUndirectedPaths{\OrientedGraph} %\neq\emptyset
        \eqfinv
...
\begin{proof} \quad 
  ...
  \( \bp{ \np{\vertex_{i-1},\vertex_{i},+1},
      \np{\vertex_{i},\vertex_{i+1}},+1} \)
  ...
  there exists \( \UndirectedPathbis =\Pair{\pathbis, -\1_{\cardinal{\pathbis}} } 
  ...
  there exists \( \UndirectedPathter =\Pair{\pathter,  \1_{\cardinal{\pathter}} }
  ...
  \( \UndirectedPathbis = \bPair{\pathbis, -\1_{\cardinal{\pathbis}} } \) and 
  \( \np{\dgent, \vertexter } \) is the last edge of the \undirectedEdgePath\ 
  \( \UndirectedPathter =\bPair{\pathter,  \1_{\cardinal{\pathter}} } \).
  ...
  and \( \orient %\np{-1,\ldots,+1}
  \in\na{-1}\times\na{-1,+1}^{\cardinal{\UndirectedPath}-2}\times\na{+1} \)
  such that \( \Pair{\path, \orient } \in \DeploymentInUPaths{\np{\bgent,\cgent}}{\OrientedGraph}
  \cap \ActiveUndirectedPaths{\OrientedGraph} \)
  \ifpreprint
  and that
  \(\Internals_{\UPATH}\np{\UndirectedPath} \subset
  ...
  and that 
  \( \UndirectedPath_i = \Pair{\path_i, \orient_i }
  ...
  \[ \UndirectedPath = \Pair{\path_0 \ltimes \cdots \ltimes \path_n,
  ...
  \begin{align}
    \bp{ \np{\tail{\vertex_{i}},\dgent_{i+1},+1},  \np{\dgent_{i+1}, \head{\vertex_{i+1}},-1}},
    &= 
      {\Head{\ProjectionUndirectedPath}\np{\UndirectedPath_{i}}
      \ltimes
  ...
  \( \UndirectedPath_i = \Pair{\path_i, \orient_i } \) and 
  \( \np{\dgent_{i+1}, \head{\vertex}_{i+1}} \) is the first edge of the \undirectedEdgePath\ 
  \( \UndirectedPath_{i+1} =\Pair{\path_{i+1}, \orient_{i+1} } \).
...
\begin{proof}
  ...
  \item
    ...
    that there exists
    \[
      \UndirectedPath_{1}= \Pair{\path_{1}, \orient_{1} }
      =\Pair{\path_{1}, \np{\ldots,+1} } \in 
  ...
  it only remains to show that
  \begin{align}
    \bp{\np{\tail{\vertex_{1}},\dgent_{1},+1},  \np{\dgent_{1}, \head{\vertex_{1,2}},-1 }}
    &= 
      {\Head{\ProjectionUndirectedPath}\np{\UndirectedPath_{1}}
      \ltimes
    ...
    % 
      \bp{  \np{\tail{\vertex_{1,2}},\dgent_{2},+1 }, \np{\dgent_{2}, \head{\vertex_{2}},-1 } }
    &= 
      {\Head{\ProjectionUndirectedPath}\np{\UndirectedPath_{1,2}}
      \ltimes
      \Tail{\ProjectionUndirectedPath}\np{\UndirectedPath_{2}}}
...
\begin{proof}
  ...
  \undirectedEdgePath~$\overline{\UndirectedPath}=
  ...
  \item
    ...
    \( \bp{ \np{\tail{\vertex_{n-1}},\head{\vertex_{n-1}}, -1},
      \np{\tail{\vertex_n},\head{\vertex_n}, -1} } \)
    must satisfy Item~\ref{it:ActivePaths_case2} in
    Definition~\ref{de:ActivePaths}. We deduce that 
    $\head{\vertex_{n-1}}=\tail{\vertex_{n}} \in \Complementary{\AgentSubsetW}$. 
    Now, we wrap up the results obtained so far.
    % 
