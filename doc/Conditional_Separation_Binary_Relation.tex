...
\cite{Gonthier-Mahboubi-Tassi:2016}
\cite{MathComp:2022}

{\small 
\inputsnippets{ssrel/relation.tex}
\inputsnippets{ssrel/clos_refl_trans.tex}
\inputsnippets{ssrel/clos_rt.tex}
}
...
to $n$ of elements of the set $\Set$, that is the disjoint unions
$\sqcup_{k\ge n} \Sequence_k(\Set)$.

Now, for $n\ge 2$, we consider a lift operator $\Lift_n: \Sequence_{n}(\VERTEX)\to 
\Sequence_{n{-}1}(\VERTEX{\times}\VERTEX)$ transforming a sequence of nodes of length $n$
into a sequence of edges of length $n{-}1$ as follows 
\begin{align}
  \forall (\vertex_1,\dots, \vertex_n)\in \SetProd{\VERTEX}{n}
  \eqsepv
  \Lift^n(\vertex_1,\dots, \vertex_n) = \bp{\np{\vertex_1, \vertex_2},\np{\vertex_2, \vertex_3},\dots,
...
\begin{equation}
  ...
  \subset \Sequence_{n}(\VERTEX{\times}\VERTEX))
  \eqfinp
\end{equation}
It easily follows from the definition of $\PATH^n\np{\OrientedGraph}$ that we also have
\begin{align}
...
$\Lift: \Sequence_{\ge 2}(\VERTEX) \to \Sequence_{\ge 1}(\VERTEX{\times}\VERTEX)$ such that for all $n \ge 2$ the restriction of
$\Lift$ to the set $\Sequence_{n}(\VERTEX)$ coincide with the $\Lift_n$ mapping, that is ${\Lift}_{\vert\Sequence_n(\VERTEX)}= \Lift_n$.
The set of finite (edge) paths of length greater of equal to $1$ is defined by
\begin{align}
  \PATH^{\ge 1}\np{\OrientedGraph}
  &=
    \Lift\bp{ \Sequence_{\ge 2}(\VERTEX)} \cap \Sequence_{\ge 1}(\EDGE)
    \subset \Sequence_{\ge 1}(\VERTEX{\times}\VERTEX))
  \eqfinv
\end{align}
...
a set in $T*T$, $\EDGE:\texttt{set T*T}$ using the set definition of mathcomp analysis. We then directly define the List operator $\Lift$ as follows.
...
\noindent $\bullet$
An \emph{\undirectedEdgePath} of length~$n$ ($n\ge 1$) relative to the graph~$\npOrientedGraph$
is a couple 
\( \UndirectedPath= \np{\path,\orient} \in 
\SetProd{(\EDGE \cup \Converse{\EDGE})}{n} \times \na{-1,+1}^n \),
where \( \path=\bseqa{ \np{\tail{\vertex_i},\head{\vertex_i}} }{i\in\ic{1,n}} \) 
and $\orient = \sequence{\orient_i}{i\in\ic{1,n}} \in \na{-1,+1}^n$
are such that, for all $i\in\ic{1,n{-}1}$, we have \( \head{\vertex_i}=\tail{\vertex_{i+1}}
\) and 
either $\np{\tail{\vertex_i},\head{\vertex_i}}\in \EDGE$ if $\orient_i={+1}$
or $\np{\tail{\vertex_i},\head{\vertex_i}}\in \Converse{\EDGE}$ if $\orient_i={-1}$.
We denote 
\begin{subequations}
  \begin{align}
    \EDGE^{(+1)}
    &= \EDGE \eqsepv \EDGE^{(-1)}= \Converse{\EDGE}
      \intertext{and we introduce (for $n\ge 1$)}
      % \\
    \UPATH^n\np{\OrientedGraph}
    &= \Big\{ \np{\path,\orient} \in \SetProd{(\EDGE\cup\Converse{\EDGE})}{n} \times
      \na{-1,+1}^n \, \big\vert 
      \nonumber
    \\
    & 
      \qquad \path=\sequence{\path_i}{i\in\ic{1,n}} \in\PATH^n\np{\UnorientedGraph} \text{ and } \path_i \in
      \EDGE^{(\orient_i)} \text{ for } i\in\ic{1,n} \Big\}
      \label{eq:undirected_(edge)_paths_of_length_n}
    \\
    &\subset  \SetProd{\np{\EDGE\cup \Converse{\EDGE}}}{n} \times \na{-1,+1}^n
      \nonumber 
      \intertext{the set of \undirectedEdgePaths\
      of length~$n$ ($n\ge 1$) relative to the graph~$\npOrientedGraph$, and by}
      \UPATH\np{\OrientedGraph}
    &=
      \bigcup_{n\ge 0} \UPATH^n\np{\OrientedGraph}
      \subset \bp{ \VERTEX^2\times\na{+1} } \cup \bigcup_{n\ge 1} \Bp{ 
      \SetProd{(\EDGE\cup\Converse{\EDGE})}{n} \times \na{-1,+1}^n }
      \label{eq:undirectedEdgePaths}
  \end{align}
  % with the notation $\EDGE^{(+1)} = \EDGE$ and $\EDGE^{(-1)}= \Converse{\EDGE}$ and by
  % $\UPATH\np{\OrientedGraph}= \bigcup_{n\ge 1} \UPATH^n\np{\OrientedGraph}$
  the set of finite \undirectedEdgePaths\footnote{%
    ...
    % Following an \undirectedEdgePath\ is just as following a route with your car in a town allowing yourself to take one way
    % road in the forbidden direction. 
    However, considering a couple $(\path,\orient)\in \UPATH^n\np{\OrientedGraph}$, we obtain that
    $\path \in \PATH^n\np{\UnorientedGraph}$, % with $\hat{\EDGE}= \EDGE \cup \Converse{\EDGE}$, 
    that is, $\path$ is an (edge) path in the unoriented graph
    \( \npUnorientedGraph \).
    % \hat\graph = (\VERTEX, \EDGE \cup \Converse{\EDGE})$.
    We thus obtain a natural surjection
    $(\path,\orient)\mapsto \path$
    % $\injection_{\UPATH}: (\path,\orient)\mapsto \path$ 
    ...
    when no two edges have the same endpoints. 
    % For $\path\in \PATH_{\hat{\EDGE}}\np{\UnorientedGraph}$, we have that
    % For $\path\in \PATH\np{\UnorientedGraph}$, we have that
    % \( \injection_{\UPATH}^{-1}(\path)
    % =  \bset{ (\path,\orient) }{ \orient \in \na{-1,+1}^{\cardinal{\path}}} \).
    % \begin{equation}
    %   \injection_{\UPATH}^{-1}(\path)
    %   =  \bset{ (\path,\orient) }{ \orient \in \na{-1,+1}^{\cardinal{\path}}}
    %   \eqfinp
