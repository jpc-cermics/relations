...
\inputsnippets{ssrel/relation.tex,ssrel/clos_refl_trans.tex,ssrel/clos_rt.tex}
...
\begin{itemize}
  \item graph~\( \npOrientedGraph \).
  \item Relations on $\VERTEX$ are defined with caligraphic letters as in~${\cal R}$
  \item Product space $\VERTEX_{(2)} = \VERTEX{\times}\VERTEX$. $\VERTEX_{(4)} = (\VERTEX{\times}\VERTEX)_{(2)}$
  \item Relations on $\VERTEX$ are subsets of $\VERTEX_{(2)}$ and denoted as~${\cal R}_{(2)}$. Note that
    $\VERTEX_{(2)}$ is indeed also a relation on $\VERTEX$.
  \item Sequences ${\mathbb S}[\VERTEX]$, with also ${\mathbb S}_{\ge k}[\VERTEX]$ and ${\mathbb S}_{k}[\VERTEX]$.
  \item Sequences ${\mathbb S}[\VERTEX_{(2)}]$, with also ${\mathbb S}_{\ge k}[\VERTEX_{2}]$ and ${\mathbb S}_{k}[\VERTEX_{(2)}]$.
  \item Lift operator $\Lift: {\mathbb S}[\VERTEX] \to {\mathbb S}[\VERTEX{\times}\VERTEX]$
  \item Lift operator $\Lift_{(2)}: {\mathbb S}[\VERTEX_{(2)}] \to {\mathbb S}[(\VERTEX{\times}\VERTEX)_{(2)}]$.
  \item Given a set $\Set$ and a subset $\Setbis$ of $\Set$
    Then ${\mathbb S}[\Set,\Setbis]$ is the subset of ${\mathbb S}[\Set]$.
  \item Given a set $\Set$ and a relation ${\cal R}$ on $\Set$.
    Then, ${\mathbb L}[\Set;{\cal R}]= {\cal L}\bp{{\mathbb S}[\Set]} \cap {\mathbb S}[\Set_{(2)},\cal R]$
...
\noindent $\bullet$ Definition of (edge) paths. We first introduce some notations.

For any set ${\Set}$, we denote by $\Sequence_n(\Set)$ the set of sequences of
length $n$ of elements of the set~$\Set$, $\Sequence_n(\Set)= \SetProd{\Set}{n}$
and by $\Sequence_{\ge n}(\Set)$ the set of finite sequences of length greater or equal
to $n$ of elements of the set $\Set$, that is the disjoint union
$\sqcup_{k\ge n} \Sequence_k(\Set)$.

Now, for $n\ge 2$, we consider a lift operator $\Lift_n: \Sequence_{n}(\Set)\to 
\Sequence_{n{-}1}(\Set{\times}\Set)$ transforming a sequence of elements of
$\Set$ of length $n$
into a sequence of oriented pairs in $\Set\times\Set$ of length $n{-}1$ as follows 
\begin{align}
  \forall (\vertex_1,\dots, \vertex_n)\in \SetProd{\Set}{n}
  \eqsepv
...
$\Lift: \Sequence_{\ge 2}(\Set) \to \Sequence_{\ge 1}(\Set{\times}\Set)$ such that for all $n \ge 2$ the restriction of
$\Lift$ to the set $\Sequence_{n}(\Set)$ coincide with the $\Lift_n$ mapping, that is ${\Lift}_{\vert\Sequence_n(\Set)}= \Lift_n$.
...
We describe the coq formalization of (edge) paths. A graph $(\VERTEX,\EDGE)$ is given by a type \texttt{T: Type} and a relation on \texttt{T} which is
a set in $T*T$, $\EDGE:\texttt{set T*T}$ using the (classical) set definition of mathcomp analysis. We then directly define the \texttt{Lift} operator $\Lift$ as follows.

{\small 
\inputsnippets{erel3/Lift.tex}
}
Checking that all the elements of a sequence \texttt{(p: seq T)} belongs to a set \texttt{(X: set T)} is done by an inductive predicate \texttt{All}.
Then, we prove in a lemma that the inductive predicate \texttt{All} can be equivalently defined with the help of the function
\texttt{all} of mathcomp sequence library \texttt{seq.v}.
{\small
  ZZZZ %% \inputsnippets{erel3/All_notation.tex, erel3/All_with_all.tex}}
\medskip
Finally, the set of finite (edge) paths of length greater of equal to $1$, $\PATH^{\ge 1}\np{\OrientedGraph}$ defined in Equation~\eqref{def:epaths_gt1} is defined by
{\small
  \inputsnippets{erel3/EPath1.tex}
}
Thus in the Coq implementation an (edge) path is given as a node path \texttt{p} satisfying a predicate \texttt{All (Lifp p)}.
...
values \texttt{N} and \texttt{P} which will serve as an orientation
specification of an edge {\small \inputsnippets{erel3/O.tex}} Now, for $n\ge 2$,
we consider an oriented lift operator
$\Lifto_n: \Sequence_{n}(\Set)\times\Sequence_{n-1}(\Orientation) \to
\Sequence_{n{-}1}(\Set{\times}\Set{\times}\Orientation)$ transforming a sequence of elements
of $\Set$ of length $n$ and a sequence of orientations of length $n{-}1$ into a
sequence of oriented triples in $\Set{\times}\Set{\times}\Orientation$ of length
$n{-}1$ as follows
\begin{align}
  \forall \vertex\in \Sequence_n(\Set)
  \eqsepv
...
$$\Lifto: \sqcup_{n\ge 2} \Sequence_{n}(\Set){\times}\Sequence_{n-1}(\Orientation)
\to \Sequence_{\ge 1}(\Set{\times}\Set{\times}\Orientation)$$ such that for all
$n \ge 2$ the restriction of $\Lifto$ to the set
$\Sequence_{n}(\Set){\times}\Sequence_{n-1}(\Orientation)$ coincide with the
$\Lifto_n$ mapping. 
...
follows in Coq
{\small \inputsnippets{erel3/pair.tex,erel3/LiftO.tex}}
Moreover, we have to
...
\inputsnippets{erel3/EoPath1.tex}
...
\begin{subequations}
  ...
  \begin{equation}
    ...
    \label{eq:Projection}
  \end{equation}
  % \mdl{cette notation ne semble pas utilisée}
  % \rouge{  We also distinguish the tail and the head endpoints by
  % \begin{equation}
  %   \Projection_{\UPATH}=\np{\Tail{\Projection_{\UPATH}},\Head{\Projection_{\UPATH}}}
  %   \mtext{ where }
  %   \Tail{\Projection_{\UPATH}}: \UPATH \np{\OrientedGraph} \to \VERTEX
  %   \mtext{ and }
  %   \Head{\Projection_{\UPATH}}: \UPATH \np{\OrientedGraph} \to \VERTEX
  %   \eqfinp 
  % \end{equation}  }
\end{subequations}
\medskip
