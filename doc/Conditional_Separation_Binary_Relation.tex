...
We have two ways to characterize the set $\Lift\bp{ \Sequence_{>1}(\fullset)}$. 
First as the image through the lift operator of the set $\Sequence_{> 1}(\fullset)$ 
{\small \inputsnippets{erel3/IChains.tex}}
Second, as a preimage as $\Lift^{-1}\bp{ \Sequence_{> 1}(Ch)}$
{\small \inputsnippets{erel3/BChains.tex}}
using a relation \texttt{Chrel} on the set $T{\times}T$  defined by 
{\small \inputsnippets{erel3/Chrel} 

These two sets are proved to be the same in \texttt{Lemma XXX}.
