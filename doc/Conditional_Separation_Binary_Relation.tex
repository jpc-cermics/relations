...
definitions recalled at the beginning of this section.\coqend
...
\texttt{T} and the restriction to elements in a subset \texttt{(\PY{n+nv}{D}:
  set T)} is obtained using the function \texttt{all} (in mathcomp mathcomp
library\texttt{seq.v}).  As an example, the set $\Sequence_{n}(D)$ is
implemented as follows {\small \inputsnippets{seq1/all_notation-n.tex,seq1/Sn-n.tex}}\coqend
...
\coqstart The lift operator $\Lift$ is implemented in Coq as a recursive mapping
...
\paragraph{$\bullet$ \texttt{Lift} bijection.}
\coqstart The lift operation, when restricted to the subset \texttt{D} defined below,
is bijective onto its image \texttt{I}
{\small \inputsnippets{seq1/DI-n.tex,seq1/Lift_inj-n.tex,seq1/Lift_surj-n.tex}}
The inverse of \texttt{Lift} is obtained with the recursive mapping \texttt{UnLift} (not detailed here). \coqend
...
\coqstart A graph $(\VERTEX,\EDGE)$ is given in Coq by a type \texttt{(T: Type)}
...
\coqstart Using the tools introduced in \S\ref{sec:sequences} and Equations~\eqref{def:epaths_defs} we obtain 
...
we defined in Section~\ref{sec:sequences}. \coqend
...
\coqstart The endpoints are obtained in Coq by the mapping \texttt{Pe} for a sequence of vertices and by the mapping 
\texttt{Epe} for a sequence of edges
{\small \inputsnippets{seq1/Pe-n.tex,seq1/Epe-n.tex}}\coqend

\paragraph{$\bullet$ Transitive closure and edge paths.} As a first result linking
(edge) paths and relations we prove that {\small
  \inputsnippetsE{erel3/TCP-n.tex}{,} which asserts that two nodes
...
\paragraph{$\bullet$ Deployment in paths.}
...
\coqstart {\small \inputsnippets{seq1/D_P-n.tex}} 
...
intermediate nodes \texttt{p} of the \EdgePath\ \texttt{st} are immediately obtained.\coqend
...
\paragraph{$\bullet$ Definition of endpoints.} We start by considering the mapping \(
...
\begin{subequations}
  ...
  \end{equation}
\end{subequations}
\coqstart The Coq definition follows {\small \inputsnippetsE{erel3/Eope.tex}{,}}
the projection $\pi$ being implement by the mapping \texttt{unpair}. \coqend

\paragraph{$\bullet$ Deployment in paths.} Now, with any binary relation \( \relation \subset \VERTEX\times\VERTEX \),
...
binary relation~\( \relation \).
{\small \inputsnippets{erel3/D_U-n.tex}}
...
\begin{definition}[active \undirectedEdgePaths\ \( \ActiveUndirectedPaths{\OrientedGraph}\)]
  ...
  Finally, we define by $\ActiveDeploymentInUPaths{\relation}{\OrientedGraph}$ 
  the subset $\DeploymentInUPaths{\relation}{\OrientedGraph}\cap \ActiveUndirectedPaths{\OrientedGraph}$.
\end{definition} 
Notice that any \undirectedEdgePath\ of length~1 is active by definition.

\coqstart The Coq formalization of the binary relation \(\ActiveTr\) easily follows 
{\small \inputsnippetsE{erel3/A_tr.tex}{,}}
...
is defined in Coq by \texttt{Fset}. \coqend
\medskip

\coqstart Now, The Coq formalization of $\ActiveDeploymentInUPaths{\relation}{\OrientedGraph}$ is 
{\small \inputsnippets{erel3/D_U_a-n.tex}} \coqend
