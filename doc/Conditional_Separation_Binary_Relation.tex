...
definitions recalled at the begining of this section.
% ,ssrel/clos_refl_trans.tex,ssrel/clos_rt.tex}
\subsection{Sequences sets and relations}
\label{sec:sequences}
\newcommand{\fullset}{T}

To formalize in Coq graph paths we use sequences (defined in mathcomp
...
notations. We consider a set $\fullset$, and for any set ${\Set} \subset \fullset$ and $n\in \NN$,
we denote by
...
\paragraph{(Coq)} The sets $\Sequence_{\ge n}(\fullset)$ and
$\Sequence_{n}(\fullset)$ are implemented in Coq using sequences of elements of
type \texttt{T} and the restriction to elements in a subset
\texttt{(\PY{n+nv}{D}: set T)} is obtained using the function \texttt{all}.  As
an example $\Sequence_{n}(D)$ is implemented as follows {\small
  \inputsnippets{erel3/all_notation.tex,erel3/Sn.tex}}
...
\[ \Lift: \Sequence(\fullset) \to \Sequence(\fullset{\times}\fullset)
  \eqfinv\] such that for all $n \ge 2$ the restriction of $\Lift$ to the set
$\Sequence_{n}(\fullset)$ coincide with the $\Lift_n$ mapping, that is
${\Lift}_{\vert\Sequence_n(\fullset)}= \Lift_n$ and for ${\Lift}\bp{\Sequence_{0}(\fullset) \cup\Sequence_{1}(\fullset)}$ is a singleton set composed of the empty list on $\fullset{\times}\fullset$.

The lift operator $\Lift$ is implemented in Coq as a recursive mapping denoted
\texttt{Lift}: {\small \inputsnippetsE{erel3/Lift.tex}{,} }
it will be usefull, in this work, to convert node paths into edge paths and
conversely and to check that edge paths satisfy given relations between successive
edges. As an example, using the relation $C_{\textsc{H}}$ -- denote
\texttt{Chrel} in Coq -- on the product set $T{\times}T$ defined by
$((v_1, v_2)\, C_{\textsc{H}}\, (v_3 v_4) \iff v_2 = v_3)$, that is {\small
  \inputsnippetsE{erel3/Chrel}{,}} we obtain four equivalent ways -- two are
...
$C_{\textsc{H}}$ relation. We obtain that \texttt{(Im 2) = (Im1 2) = (Pre 0) = (Pre1 1)}, 
where the four paths sets are defined by 
{\small \inputsnippets{erel3/Lift_lemma2.tex}}
...
{\small \inputsnippets{erel3/Lift_lemma}}
...
A graph $(\VERTEX,\EDGE)$ is given in Coq by a type \texttt{(T: Type)} and a relation on
...
\noindent $\bullet$ Definition of \emph{\EdgePath}.
...
\begin{equation}
  \PATH_n\np{\OrientedGraph} =
  \Lift \bp{ \Sequence_{n+1}(\VERTEX)} \cap \Sequence_{n}(\EDGE)
  \eqsepv
\end{equation}
and satisfy $\PATH_n\np{\OrientedGraph} \subset \Sequence_{n}(\EDGE)$$ \subset \Sequence_{n}(\VERTEX{\times}\VERTEX))$.
Moreover, it easily follows from the definition of $\PATH_n\np{\OrientedGraph}$ that we also have the followin equality
\begin{align}
...
\begin{align}
  \PATH_{\ge n}\np{\OrientedGraph}
  &=
    \Lift\bp{ \Sequence_{\ge n+1}(\VERTEX)} \cap \Sequence_{\ge n}(\EDGE)
    \label{def:epaths_gt1}
    \eqfinv
\end{align}
and satisfy $\PATH_{\ge n}\np{\OrientedGraph} \subset \Sequence_{\ge n}(\EDGE)$$ \subset \Sequence_{\ge n}(\VERTEX{\times}\VERTEX)$
and coincide with the set $\sqcup_{n'\ge n} \PATH_{n'}\np{\OrientedGraph}$.

Using the tools introduced in Section~\ref{sec:sequences} and Equation~\eqref{def:epaths_gt1} we obtain in Coq
the following three equivalent definitions of $P_{\ge 1}$
{\small \inputsnippetsE{erel3/EPath2new.tex,erel3/EPath3new,erel3/EPath2inter}{,}}
where \texttt{`\&`} is the set intersection as defined in \texttt{classical\_sets.v}.
...
As a first result linking (edge) paths and relations we can prove that 
{\small \inputsnippetsE{erel3/TCP.tex}{,}
