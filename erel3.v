...
Section Active_relation.
  ...
  Definition D_U_a (R E: relation T) (W: set T):= [set stto |size(stto)>0 
     /\ R (Eope ptv stto ) /\ stto [\in] (Oedge E) 
  ...
  Lemma D_U_a_eq4 : forall (R E: relation T) (W: set T),
      [set stto | size(stto) > 0 /\ R (Eope ptv stto ) /\ stto [\in] (Oedge E) /\ stto [L\in] (A_tr W E)]
      = [set stto | R (Eope ptv stto )] `&` 
          [set stto | size(stto) > 0 /\ stto [\in] (Oedge E) /\ stto [L\in] (A_tr W E)].
  Proof. 
    move => R E W.
    rewrite /setI /mkset predeqE => stto.
    by rewrite andA [0 < size stto /\ R (Eope ptv stto)]andC -andA.
  Qed.
  ...
  Lemma D_U_a_eq6 : forall (R E: relation T) (W: set T),
      [set stto | size(stto) > 0 /\ R (Eope ptv stto ) /\ stto [\in] (Oedge E) /\ stto [L\in] (A_tr W E)]
      = ( [set stto | R (Eope ptv stto )] `&`  [set stto | size(stto) = 1 /\ (stto [\in] (Oedge E)) ] )
          `|`
          ([set stto | R (Eope ptv stto )] `&`  [set stto | size(stto) > 1 /\ stto [L\in] (((Oedge E) `*` (Oedge E))`&`(A_tr W E) )] 
          ).
  Proof. 
    move => R E W.
  ...
  Lemma D_U_a_eq7 : forall (R E: relation T),
       [set stto | R (Eope ptv stto )] `&`  [set stto | size(stto) = 1 /\ (stto [\in] (Oedge E)) ] 
       = [set stto | size(stto) = 1 /\ R (Eope ptv stto ) /\ (stto [\in] (Oedge E)) ].
  Proof.
    move => R E.
    rewrite /setI /mkset predeqE => stto.
    by rewrite andA [ R (Eope ptv stto) /\ size stto = 1]andC -andA.
  Qed.

  Lemma D_U_a_eq8 : forall (R E: relation T) (W: set T),
      [set stto | R (Eope ptv stto )] `&` 
        [set stto | size(stto) > 1 /\ stto [L\in] (((Oedge E) `*` (Oedge E))`&`(A_tr W E) )]
      = [set stto | size(stto) > 1 /\ R (Eope ptv stto ) /\  stto [L\in] (((Oedge E) `*` (Oedge E))`&`(A_tr W E) )].
  Proof. 
    move => R E W.
    rewrite /setI /mkset predeqE => stto.
    by rewrite andA [ R (Eope ptv stto) /\ size stto > 1]andC -andA.
  ...
  Lemma D_U_a_eq10 : forall (R E: relation T) (W: set T),
      ...
      =
        [set stto | size(stto) = 1 /\ R (Eope ptv stto ) /\ (stto [\in] (Oedge E)) ]
          `|` 
          [set stto | size(stto) > 1 /\ R (Eope ptv stto ) /\  stto [L\in] (A_tr_eo W E)].
  Proof. 
    move => R E W.
  ...
  Lemma D_U_a_eq11 : forall (R E: relation T) (W: set T),
      ...
      =
        [set stto | size(stto) = 1 /\ R (Eope ptv stto ) /\ (stto [\in] (Oedge E)) ]
          `|` 
          [set stto | size(stto) > 1 /\ R (Eope ptv stto ) /\ stto [L\in] (ActiveOe W E)].
  Proof. 
  ...
  Lemma D_U_a_eq12:  forall (E: relation T) (W: set T) (stto: seq (T*T*O)) (x y:T),
      size stto > 1 /\ (Eope ptv stto) = (x,y) /\ stto [L\in] (ActiveOe W E) 
  ...
  Proof.
    ...
    - pose proof seq_cc H1 as [q [eo1 [eo2 ->]]].
      exists q;exists eo1; exists eo2.
      move: H2; rewrite Eope_L1. move => [H2 H4].
