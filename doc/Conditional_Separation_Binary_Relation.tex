...
Moreover, the inverse of \texttt{Lift} is explicitely obtained by a recursive mapping \texttt{UnLift} (not detailed here). \coqend
...
\paragraph{$\bullet$ Definition of \EdgePath.}
...
{\small \inputsnippets{seq1/Epe_Lift-n.tex,seq1/Pe_UnLift-n.tex}}\coqend
...
\begin{subequations}
  ...
  \end{equation}
\end{subequations}
\coqstart The Coq definition follows {\small \inputsnippetsE{seq1/Eope.tex}{,}}
\noindent the projection $\pi$~in Equation~\eqref{eq:Projection_UPATH} 
being implement by the mapping \texttt{unpair}. 

\coqend

\paragraph{$\bullet$ Deployment in \undirectedEdgePaths.} Now, with any binary relation \( \relation \subset \VERTEX\times\VERTEX \),
...
\coqstart It is formalized in Coq as follows
...
\begin{definition}[active \undirectedEdgePaths\ \( \ActiveUndirectedPaths{\OrientedGraph}\)]
  ...
  as the subset $\DeploymentInUPaths{\relation}{\OrientedGraph}\cap \ActiveUndirectedPaths{\OrientedGraph}$.
\end{definition} 
...
\coqstart Now, The Coq formalization of $\ActiveDeploymentInUPaths{\relation}{\OrientedGraph}$ is obtained as an intersection of two sets 
...
{\small \inputsnippets{seq1/D_U_a1-n.tex}} \coqend
...
\coqstart We give here, as an example, the Lemma~\ref{le:L7} as written in Coq
