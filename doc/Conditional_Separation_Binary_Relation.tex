...
\paragraph{$\bullet$ \texttt{p [\in X]}.} We consider a set $\fullset$, and for any set ${\Set} \subset \fullset$ and $n\in \NN$,
we denote by
...
{\small \inputsnippets{erel3/all_notation-n.tex,seq1/Sn-n.tex}}\coqend

\paragraph{$\bullet$ \texttt{Lift p}.} Then, we define a lift operator
...
\paragraph{$\bullet$ \texttt{p [Suc\in] R}.} It will be usefull, in this work, to
to check that successive elements of a sequence whose elements are in $\fullset$ belong to a subset of $\fullset\times\fullset$ that is satisfy a relation on $\fullset$.

As an example, using the
relation $C_{\textsc{H}}$ -- denote \texttt{Chrel} in Coq -- on the product set
$T{\times}T$ defined by
...
$C_{\textsc{H}}$ relation. We obtain that \texttt{(Im 2) = (Im1 2) = (Pre 0) =
  ...
  \inputsnippets{erel3/Lift_lemma2-n.tex}}
