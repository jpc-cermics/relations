...
\coqstart Using the tools introduced in Section~\ref{sec:sequences} and Equations~\eqref{def:epaths_gt1_}-\eqref{def:epaths_gt1} we obtain 
the following formalization of $\PATH_{> n}$
{\small \inputsnippetsE{seq1/EPath_gt-n.tex}{,}}
\noindent where \texttt{E} is the edge relation $\EDGE$ and where \texttt{Chrel} we defined in Section~\ref{sec:sequences}. \coqend
...
subset of sequences of vertices:
...
intermediate nodes \texttt{p} of the \EdgePath\ \texttt{sp} are immediately obtained.\coqend
\subsubsection{Extended-oriented paths in a (directed simple) graph (permitting loops)}

To define \emph{\undirectedEdgePath}, we consider a finite set ${\Orientation}=\na{-1,+1}$
...
\footnote{%
  ...
  \end{subequations}
  \coqstart Using the tools introduced in Section~\ref{sec:sequences} and
  Equations~\eqref{def:eopaths_defs} we obtain the following formalization of
  $\UPATH_{> n}$ {\small
...
\paragraph{parler de U construit comme image avec LiftO}
